...
def launch_model_split(config: dict, config_filename: str = 'config.txt') -> None:
    ...
    if int(config.get('batch_size', 1)) < len(gpu_ids):
        print(
            f"[model_split] note: batch_size={config.get('batch_size')} < num_stages={len(gpu_ids)}. "
            "Pipeline runs sequentially for memory savings."
        )
...
def _forward_step(stage: ModelSplitStage, graph, device):
    ...
    *, stage: ModelSplitStage, loader, optimizer, device, config,
    ...
    for graph in loader:
        ...
        with torch.amp.autocast('cuda', dtype=amp_dtype, enabled=use_amp):
            sync_loss, batch_loss, batch_count = _forward_step(stage, graph, device)
